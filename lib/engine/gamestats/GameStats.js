...
export default class GameStats {
  ...
  init() {
    ...
    this.canvas.style.cssText = `width:${this.config.baseCanvasWidth * this.config.scale}px;height:${
      this.config.baseCanvasHeight * this.config.scale
    }px;background-color:${this.config.COLOR_BG}`
  ...
  initPixi() {
    this.extensions.pixi = new PixiStats(this)
  }
  ...
  update() {
    ...
    if (
      performance &&
      performance.memory &&
      performance.now() - this.lastMemoryMeasure > this.config.memoryUpdateInterval
    ) {
  ...
  drawLines() {
    ...
    else if (average > targetFPS * 1.33) ctx.fillStyle = config.COLOR_TEXT_TOO_LOW
  ...
  drawFPS() {
    ...
    else if (FPS < config.targetFPS * 0.66) ctx.fillStyle = config.COLOR_TEXT_TOO_LOW
    ctx.font = `${fontSize}px ${config.FONT_FAMILY}`
    ctx.textAlign = 'right'
    ...
    else if (averageFPS < toLowFPS) ctx.fillStyle = config.COLOR_TEXT_TOO_LOW
    ...
    else if (minFPS < toLowFPS) ctx.fillStyle = config.COLOR_TEXT_TOO_LOW
    ...
    else if (maxFPS < toLowFPS) ctx.fillStyle = config.COLOR_TEXT_TOO_LOW
  ...
  drawMemory() {
    ...
    const padding = config.baseCanvasHeight * 0.01
    const memoryTextY = config.baseCanvasHeight * 0.6
    // avg min max
    ctx.textAlign = 'left'
    let fontSize = config.baseCanvasWidth * 0.09
    ctx.font = `${fontSize}px ${config.FONT_FAMILY}`
    ...
    if (allocatedMemory > reservedMemory * 0.9) {
      ...
      ctx.fillStyle = config.COLOR_TEXT_TOO_LOW
    }
    ...
    else if (average > targetMemory * 0.66) ctx.fillStyle = config.COLOR_TEXT_TOO_LOW
    ...
    ctx.fillRect(
      this.memoryGraph.width - this.memoryGraph.barWidth * 6,
      averageY,
      this.memoryGraph.barWidth * 6,
      this.memoryGraph.barWidth * 6
    )
